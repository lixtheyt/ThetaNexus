...
namespace ThetaNexus
{
    ...
    {
        internal static async Task<bool> Show(Exception ex)
        {
            var root = ex;
            ...
            {
                (DockerApiException api, _) => $"The engine answered with {(int)api.StatusCode} {api.StatusCode}, so it is running but refused the request. This usually means the client is newer than the engine.",
